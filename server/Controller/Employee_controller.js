...
exports.getAllEmployee = async (req, res) => {
  try {
    ...
    const employee = await employedb.find(searchCriteria);

    res.json(employee);
  } catch (err) {
    res.status(500).send({
      message: err.message || `Error while retrieving Employee information`,
    });
  }
};
...
exports.UpdateEmployee = async (req, res) => {
  if (!req.body) {
    return res.status(404).send({
      message: err.message || `Error while updating the data cannot be empty`,
    });
  }
  try {
    ...
    if (data) {
      ...
      res.status(404).send({
        message:
          err.message ||
          `cannot update Employee with identified id ${id} or maybe user not found!`,
      });
    }
  } catch (err) {
    res
      .status(500)
      .send({ message: err.message || `error updating Employee information` });
  }
};
...
exports.deleteEmployee = async (req, res) => {
  try {
    ...
    if (!data) {
      res.status(404).send({
        message: `Cannot delete Employee with id ${id} maybe id is wrong!`,
      });
    } else {
